...
# GPU selection is driven by --gpus (container-remapped indices) plus the external
# CUDA_VISIBLE_DEVICES mask (e.g. SINGULARITYENV_CUDA_VISIBLE_DEVICES=2,3 → 0,1).
# We deliberately do NOT pin CUDA_VISIBLE_DEVICES here so that --gpus 0,1 can run
# the bi-encoder stages under DataParallel across both visible GPUs.
...
# ---------------------------------------------------------------------------
# Multi-GPU helpers (mirror rq2_dropout_sweep.py)
# ---------------------------------------------------------------------------
...
def _dp(model, gpu_ids):
    """Wrap a bi-encoder in DataParallel when more than one GPU is requested.
    The model must already be .to(cuda:gpu_ids[0]); DataParallel scatters from
    there across device_ids. Single-GPU / CPU runs return the model unchanged."""
...
def main(args):
    # parse --gpus "0,1" → [0, 1]  (container-remapped indices)
    gpu_ids = [int(g.strip()) for g in args.gpus.split(',')] if args.gpus else []
    device  = _setup_device(gpu_ids)
    if gpu_ids and len(gpu_ids) > 1:
        print(f'Device: {device}  (DataParallel across GPUs {gpu_ids}; '
              f'reranker on cuda:{gpu_ids[0]})')
    else:
    ...
    pt_index, pt_idx_to_docid = build_index_cached(
        _dp(pt_encoder, gpu_ids), pt_doc_loader, device,
    ...
    bo_query_enc = _dp(bo_query_enc, gpu_ids)
    bo_doc_enc   = _dp(bo_doc_enc,   gpu_ids)
    ...
    query_enc = _dp(query_enc, gpu_ids)
    doc_enc   = _dp(doc_enc,   gpu_ids)
...
def parse_args():
    ...
    p.add_argument('--gpus', type=str, default=None,
                   help='Comma-separated GPU indices with no spaces, e.g. --gpus 0,1 '
                        '(container-remapped). The bi-encoder doc/query encoding runs '
                        'under DataParallel across them; the LLM reranker runs on the '
                        'first GPU. Omit for single-GPU.')
